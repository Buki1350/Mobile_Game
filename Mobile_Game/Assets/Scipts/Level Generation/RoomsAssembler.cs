        ...
        for (int j = 0; j < levelGridSize - 1; j++)
        {
            for (int i = 0; i < levelGridSize - 1; i++)
            {
                if (freeWaysMatrix[i, j].right && freeWaysMatrix[i + 1, j].left)
                    Instantiate(bridge, new Vector3(LevelOrigin.x - (CellSize + CellSpace) * levelGridSize / 2 + (CellSize + CellSpace) * i + (CellSize/2 + CellSpace/2), LevelOrigin.y, LevelOrigin.z - (CellSize + CellSpace) * j + (CellSize + CellSpace) * levelGridSize / 2), Quaternion.identity);
                
                if (freeWaysMatrix[i, j].down && freeWaysMatrix[i, j + 1].up)
                    Instantiate(bridge, new Vector3(LevelOrigin.x + (CellSize + CellSpace) * i - (CellSize + CellSpace) * levelGridSize / 2, LevelOrigin.y, LevelOrigin.z + (CellSize + CellSpace) * levelGridSize / 2 - (CellSize + CellSpace) * j - (CellSize / 2 + CellSpace / 2)), Quaternion.identity);
            }
        }
